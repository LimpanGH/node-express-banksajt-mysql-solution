...
const pool = mysql.createPool({
  ...
  database: 'banksajtSolutionCodealong',
...
app.post('/users', async (req, res) => {
  ...
  try {
    ...
    const accountResult = await query('INSERT INTO accounts (user_id, amount) VALUES (?, ?)', [
      userId,
      0,
    ]);
...
app.post('/sessions', async (req, res) => {
  ...
  try {
    ...
    if (user.password === password) {
      ...
      const sessionResult = await query('INSERT INTO sessions (user_id, token) VALUES (?, ?)', [
        user.id,
        token,
      ]);
